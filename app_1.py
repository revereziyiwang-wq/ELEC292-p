...
LABEL_MAP   = {1: "walking", 0: "jumping"}   # flip if teammates encode differently
...
def extract_features(window: pd.DataFrame) -> list:
    ...
    feats += [np.mean(mag), np.std(mag), np.max(mag) - np.min(mag)]
...
class App:
    ...
    def run_offline(self):
        ...
        try:
            ...
            n_w = labels.count("walking")
            n_j = labels.count("jumping")
